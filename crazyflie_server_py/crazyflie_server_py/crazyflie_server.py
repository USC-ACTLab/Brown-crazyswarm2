...
"""Run a Crazyflie server for communicating with several Crazyflies."""
...
class CrazyflieServer(Node):
    """ROS 2 node that bridges Crazyflie firmware and ROS topics/services."""

    def __init__(self):
        """Initialize the Crazyflie ROS 2 server node."""
        super().__init__(
            'crazyflie_server',
        ...
        self.swarm.query_all_values_on_connect = self._ros_parameters['firmware_params'][
        ...
        for link_uri in self.uris:
            ...
            self.swarm._cfs[link_uri].cf.link_statistics.latency_updated.add_callback(
                partial(self._latency_callback, uri=link_uri)
            )
            self.swarm._cfs[link_uri].status['num_rx_unicast'] = 0.0
            self.swarm._cfs[link_uri].cf.link_statistics.uplink_rate_updated.add_callback(
                partial(self._uplink_rate_callback, uri=link_uri)
            )
            self.swarm._cfs[link_uri].status['num_tx_unicast'] = 0.0
            self.swarm._cfs[link_uri].cf.link_statistics.downlink_rate_updated.add_callback(
                partial(self._downlink_rate_callback, uri=link_uri)
            )
            ...
            # Check if predefined log blocks can be logged and set up
            # Crazyflie log blocks and ROS 2 publishers.
    ...
    def _init_topics_and_services(self):
        ...
        for uri in self.cf_dict:
            ...
            self.create_service(Arm, name + '/arm', partial(self._arm_callback, uri=uri))
            self.create_service(
            ...
            self.create_subscription(
                Position, name + '/cmd_position', partial(self._cmd_position_changed, uri=uri), 10
            )

            self.create_subscription(
                VelocityWorld,
                name + '/cmd_velocity_world',
                partial(self._cmd_velocity_world_changed, uri=uri),
                10,
            )
            ...
            self.create_subscription(
                FullState,
                name + '/cmd_full_state',
                partial(self._cmd_full_state_changed, uri=uri),
                10,
            )
    ...
    def _connected(self, link_uri):
        ...
        if self.swarm.connected_crazyflie_cnt == len(self.cf_dict) - 1:
            ...
            elapsed = self.time_all_crazyflie_connected - self.time_open_link
            self.get_logger().info(
                f'All Crazyflies are connected! It took {elapsed} seconds'
            )
    ...
    def _fully_connected(self, link_uri):
        """Handle the event when a Crazyflie reports fully connected."""
        ...
        if self.swarm.fully_connected_crazyflie_cnt == len(self.cf_dict) - 1:
            ...
            elapsed = self.time_all_crazyflie_connected - self.time_open_link
            self.get_logger().info(
                f'All Crazyflies are fully connected! It took {elapsed} seconds'
            )
    ...
    def _init_logging(self):
        """Set up startup log blocks, publishers, and logging services."""
        for link_uri in self.uris:
            ...
            if len(cf_handle.logging['custom_log_groups']) != 0 and cf_handle.logging['enabled']:
                for log_group_name, log_group_dict in cf_handle.logging[
                    ...
                    try:
                        ...
                        self.get_logger().info(
                            f'[{self.cf_dict[link_uri]}] Could not start log configuration,'
                            '{} not found in TOC'.format(str(e))
                        )
                    except AttributeError:
                        self.get_logger().info(
                            f'[{self.cf_dict[link_uri]}] Could not add log config, '
                            'bad configuration.'
                        )
    ...
    def _init_default_logging(self, prefix, link_uri, callback_fnc):
        """Set up one default log block and its ROS 2 publisher."""
        cf_handle = self.swarm._cfs[link_uri]
        cf = cf_handle.cf
    ...
    def _log_scan_data_callback(self, timestamp, data, logconf, uri):
        """Publish scan data retrieved from the Crazyflie."""
    ...
    def _log_pose_data_callback(self, timestamp, data, logconf, uri):
        """Publish pose data retrieved from the Crazyflie."""
        cf_name = self.cf_dict[uri]
    ...
    def _log_custom_data_callback(self, timestamp, data, logconf, uri):
        """Publish data from a custom Crazyflie log block."""
        msg = LogDataGeneric()
        msg.header.stamp = self.get_clock().now().to_msg()
        ...
        try:
            ...
            self.get_logger().info(
                'Could not publish custom {logconf.name} message, stopping custom log'
            )
    ...
    def _init_parameters(self):
        """Initialize firmware parameters and mirror them into ROS 2 params."""
            ...
            for group in sorted(p_toc.keys()):
                for param in sorted(p_toc[group].keys()):
                    ...
                    if param_value is not None:
                        ...
                        cf.param.set_value_raw(
                            name, type_cf_param_to_index[type_cf_param], param_value
                        )
                        self.get_logger().info(
                        ...
                        if set_param_to_ROS is True:
                            if (
                                type_cf_param_to_ros_param[type_cf_param]
                                is ParameterType.PARAMETER_INTEGER
                            ):
                                cf_param_value = int(cf.param.get_value(name))
                            elif (
                                type_cf_param_to_ros_param[type_cf_param]
                                is ParameterType.PARAMETER_DOUBLE
                            ):
    ...
    def _parameters_callback(self, params):
        """Apply ROS 2 parameter updates to Crazyflie firmware parameters."""
        for param in params:
            ...
            if param_split[0] == 'all':
                if param_split[1] == 'params':
                    ...
                    try:
                        for link_uri in self.uris:
                            self.swarm._cfs[link_uri].cf.param.set_value(name_param, param.value)
                        self.get_logger().info(
    ...
    def _arm_callback(self, request, response, uri='all'):
        ...
        self.get_logger().info(f'[{self.cf_dict[uri]}] Arm request is {arm_bool} ')
        if uri == 'all':
            for link_uri in self.uris:
                self.swarm._cfs[link_uri].cf.platform.send_arming_request(arm_bool)
        else:
            self.swarm._cfs[uri].cf.platform.send_arming_request(arm_bool)

        return response

    def _takeoff_callback(self, request, response, uri='all'):
        """Handle a high-level takeoff request."""
    ...
    def _land_callback(self, request, response, uri='all'):
        """Handle a high-level land request."""
        duration = float(request.duration.sec) + float(request.duration.nanosec / 1e9)
        self.get_logger().info(
    ...
    def _go_to_callback(self, request, response, uri='all'):
        """Handle a high-level go-to request."""
        duration = float(request.duration.sec) + float(request.duration.nanosec / 1e9)

        self.get_logger().info(
            '[%s] go_to(pos=%f,%f,%f m, yaw=%f rad, duration=%f s, relative=%d, group_mask=%d)'
            % (
                self.cf_dict[uri],
                request.goal.x,
                request.goal.y,
            ...
            for link_uri in self.uris:
                ...
                if not upload_result:
                    ...
                    self.swarm._cfs[link_uri].cf.high_level_commander.define_trajectory(
                        trajectory_id, offset, len(trajectory)
                    )
            if upload_success_all is False:
                response.success = False
                return response
        else:
            trajectory_mem = self.swarm._cfs[uri].cf.mem.get_mems(MemoryElement.TYPE_TRAJ)[0]
            trajectory_mem.trajectory = trajectory
            upload_result = trajectory_mem.write_data_sync()
            if not upload_result:
                self.get_logger().info(f'[{self.cf_dict[uri]}] Upload failed')
    ...
    def _start_trajectory_callback(self, request, response, uri='all'):
        ...
        self.get_logger().info(
            '[%s] start_trajectory(id=%d,timescale=%f,relative=%d, reversed=%d, group_mask=%d)'
            % (self.cf_dict[uri], trajectory_id, ts, rel, rev, gm)
        )
        if uri == 'all':
            for link_uri in self.uris:
                self.swarm._cfs[link_uri].cf.high_level_commander.start_trajectory(
                    trajectory_id, ts, rel, rev, gm
                )
        else:
    ...
    def _poses_changed(self, msg):
        """Forward motion-capture poses to Crazyflie external positioning."""
        poses = msg.poses
        for pose in poses:
            name = pose.name
    ...
    def _cmd_vel_legacy_changed(self, msg, uri=''):
        """Handle legacy velocity command updates."""
    ...
    def _cmd_position_changed(self, msg, uri=''):
        """Handle position command updates."""
        x = msg.x
        y = msg.y
        z = msg.z
        yaw = msg.yaw
    ...
    def _cmd_velocity_world_changed(self, msg, uri=''):
        """Handle world-frame velocity command updates."""
        vel = msg.vel
        vx = vel.x
        vy = vel.y
        vz = vel.z
        yaw_rate = msg.yaw_rate
        self.swarm._cfs[uri].cf.commander.send_velocity_world_setpoint(vx, vy, vz, yaw_rate)

    def _cmd_hover_changed(self, msg, uri=''):
        """Handle hover command updates from the velocity multiplexer."""
        vx = msg.vx
        vy = msg.vy
        z = msg.z_distance
    ...
    def _add_logging(self, request, response, uri='all'):
        ...
        if topic_name in self.default_log_type.keys():
            try:
                ...
                self.get_logger().info(
                    f'[{self.cf_dict[uri]}] Logging for {topic_name} has already started'
                )
                response.success = False
                return response
        else:
            try:
                ...
                self.swarm._cfs[uri].logging['custom_log_groups'][topic_name]['log_config'] = (
                    lg_custom
                )
                self.swarm._cfs[uri].logging['custom_log_groups'][topic_name]['vars'] = variables
                self.swarm._cfs[uri].logging['custom_log_groups'][topic_name]['frequency'] = (
                    frequency
                )
                ...
                self.get_logger().error(
                    f'[{self.cf_dict[uri]}] Logging for {topic_name} is already active'
                )
                response.success = False
                return response
