...
class CrazyflieServer(Node):
    def __init__(self):
        ...
        for uri in self.cf_dict:
            ...
            self.create_subscription(
                Twist,
                name + "/cmd_vel_legacy",
                partial(self._cmd_vel_legacy_changed, uri=uri),
                10,
            )
            self.create_subscription(
                Hover,
                name + "/cmd_hover",
                partial(self._cmd_hover_changed, uri=uri),
                10,
            )
            ...
            self.create_subscription(
                NamedPoseArray, "/poses", self._poses_changed, qos_profile
            )
    ...
    def _init_logging(self):
        ...
        for link_uri in self.uris:
            ...
            for default_log_name in self.default_log_type:
                prefix = default_log_name
                if (
                    cf_handle.logging[prefix + "_logging_enabled"]
                    and cf_handle.logging["enabled"]
                ):
            ...
            if (
                len(cf_handle.logging["custom_log_groups"]) != 0
                and cf_handle.logging["enabled"]
            ):
                ...
                for log_group_name, log_group_dict in cf_handle.logging[
                    ...
                    self.swarm._cfs[link_uri].logging["custom_log_publisher"][
                        ...
                        LogDataGeneric,
                        self.cf_dict[link_uri] + "/" + log_group_name,
                        10,
                    )
        ...
        for link_uri in self.uris:
            ...
            for group in sorted(p_toc.keys()):
                for param in sorted(p_toc[group].keys()):
                    ...
                    try:
                        set_param_value = self._ros_parameters["all"][
                            "firmware_params"
                        ][group][param]
                    except KeyError:
                        pass
                    try:
                        ...
                        pass
                    try:
                        set_param_value = self._ros_parameters["robots"][
                            self.cf_dict[link_uri]
                        ]["firmware_params"][group][param]
                    except KeyError:
                        pass
                    ...
                    if set_param_value is not None:
                        ...
                        if (
                            cf_log_to_ros_param[type_cf_param]
    ...
    def _upload_bezier_trajectory_callback(self, request, response, uri="all"):
        ...
        #     p = CompressedSegment(
        #         duration,
        #         piece.bezier_control_pts_x,
        #         piece.bezier_control_pts_y,
        #         piece.bezier_control_pts_z,
        #         piece.bezier_control_pts_yaw,
        #     )
        #     trajectory.append(p)
        ...
        if uri == "all":
            ...
            for link_uri in self.uris:
                ...
                if not upload_result:
                    ...
                    self.get_logger().info(
                        f"[{self.cf_dict[uri]}] Upload bezier trajectory success"
                    )
                    self.swarm._cfs[link_uri].cf.high_level_commander.define_trajectory(
                        id,
                        offset,
    ...
    def _start_trajectory_callback(self, request, response, uri="all"):
        ...
        self.get_logger().info(
            ...
            % (self.cf_dict[uri], id, ts, rel, rev, gm)
        )
        if uri == "all":
            for link_uri in self.uris:
    ...
    def _remove_logging(self, request, response, uri="all"):
        ...
        if topic_name in self.default_log_type.keys():
            ...
            try:
                ...
                for log_name in self.swarm._cfs[uri].logging["custom_log_groups"][
                    topic_name
                ]["vars"]:
                    self.destroy_publisher(
    ...
    def _add_logging(self, request, response, uri="all"):
        ...
        if topic_name in self.default_log_type.keys():
            try:
                ...
                self.swarm._cfs[uri].logging[topic_name + "_publisher"] = (
                    self.create_publisher(
                        self.default_log_type[topic_name],
                        self.cf_dict[uri] + "/" + topic_name,
                        10,
                    )
                )
                self.swarm._cfs[uri].logging[
                    topic_name + "_log_config"
