...
def generate_trajectory(data, num_pieces, approx=False):
  ...
  # print("fitting x")
  resX = scipy.optimize.minimize(func, x0, (data[:,0], data[:,1], piece_length), method="SLSQP", options={"maxiter": 30,},
    constraints=constraints
    )
  # print("fitting y")
  resY = scipy.optimize.minimize(func, x0, (data[:,0], data[:,2], piece_length), method="SLSQP", options={"maxiter": 30},
    constraints=constraints
    )
  # print("fitting z")
  resZ = scipy.optimize.minimize(func, x0, (data[:,0], data[:,3], piece_length), method="SLSQP", options={"maxiter": 30}, 
    constraints=constraints
    )

  resYaw = scipy.optimize.minimize(func, x0, (data[:,0], data[:,4], piece_length), method="SLSQP", options={"maxiter": 30}, 
    constraints=constraints
    )
